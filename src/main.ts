...
function guideDiamond(positionClass: string): string {
  return `
...
function renderWalletSection(title: string, wallets: WalletEntry[]): string {
...
function renderWalletRow(wallet: WalletEntry): string {
  return `
...
const canvas = document.querySelector<HTMLCanvasElement>("#gradient-canvas");
const noiseCanvas = document.querySelector<HTMLCanvasElement>("#noise-canvas");
...
const drawBackground = (time: number): void => {
  ...
  const leftRedField = context.createRadialGradient(
    width * 0.2,
    height * 0.56,
    0,
    width * 0.18,
  ...
  const upperWarmth = context.createRadialGradient(
    width * 0.72,
    height * 0.16,
    0,
    width * 0.7,
